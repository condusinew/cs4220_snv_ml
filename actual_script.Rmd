...
final_result_class <- predict(best_model_class, test_set)

#confusion matrix for the best model
table(test_set$TRUTH, final_result_class$predictions)
...
test_set$preds <- as.logical(final_result_class$predictions)
test_set 

final_weighted_preds <- test_set[,-(1:9)]

final_weighted_preds <- final_weighted_preds |>
  filter(preds == TRUE)

write.table(results, "weighted_RF_results",row.names = F,col.names = F, sep="\t", quote=FALSE)
write.table(final_weighted_preds, "weighted_RF_predictions",row.names = F,col.names = F, sep="\t", quote=FALSE)
```
...
write.table(results_df_2, "unweighted_RF_results",row.names = F,col.names = F, sep="\t", quote=FALSE)
write.table(final_unweighted_preds, "unweighted_RF_predictions",row.names = F,col.names = F, sep="\t", quote=FALSE)
```
...
```{r}
# get predicteds 
#VERY SORRY PROF. SKANDERUP MY LOOPS KEEP TALKING TOO LONG TO DEBUG 

preds_real1 <- test_set_u |> 
  filter(Sample_Name == 'icgc_cll-T' & PREDICT == TRUE)

truth_real1 <- test_set_u |> 
  filter(Sample_Name == 'icgc_cll-T' & TRUTH == TRUE)

write.table(preds_real1[,10:13], "preds_real1.bed",row.names = F,col.names = F, sep="\t", quote=FALSE)
write.table(truth_real1[,10:13], "truth_real1.bed",row.names = F,col.names = F, sep="\t", quote=FALSE)


#REAL2.1
preds_real2.1 <- test_set_u |> 
  filter(Sample_Name == 'icgc_mbl-T' & PREDICT == TRUE)

truth_real2.1 <- test_set_u |> 
  filter(Sample_Name == 'icgc_mbl-T' & TRUTH == TRUE)

write.table(preds_real2.1[,10:13], "preds_real2.1.bed",row.names = F,col.names = F, sep="\t", quote=FALSE)
write.table(truth_real2.1[,10:13], "truth_real2.1.bed",row.names = F,col.names = F, sep="\t", quote=FALSE)

#SYN1
preds_syn1 <- test_set_u |> 
  filter(Sample_Name == 'dream1-T' & PREDICT == TRUE)

truth_syn1 <- test_set_u |> 
  filter(Sample_Name == 'dream1-T' & TRUTH == TRUE)

write.table(preds_syn1[,10:13], "preds_syn1.bed",row.names = F,col.names = F, sep="\t", quote=FALSE)
write.table(preds_syn1[,10:13], "truth_syn1.bed",row.names = F,col.names = F, sep="\t", quote=FALSE)

#SYN2
preds_syn2 <- test_set_u |> 
  filter(Sample_Name == 'dream2-T' & PREDICT == TRUE)

truth_syn2 <- test_set_u |> 
  filter(Sample_Name == 'dream2-T' & TRUTH == TRUE)

write.table(preds_syn2[,10:13], "preds_syn2.bed",row.names = F,col.names = F, sep="\t", quote=FALSE)
write.table(preds_syn2[,10:13], "truth_syn2.bed",row.names = F,col.names = F, sep="\t", quote=FALSE)

#SYN3
preds_syn3 <- test_set_u |> 
  filter(Sample_Name == 'dream3-T' & PREDICT == TRUE)

truth_syn3 <- test_set_u |> 
  filter(Sample_Name == 'dream3-T' & TRUTH == TRUE)

write.table(preds_syn3[,10:13], "preds_syn3.bed",row.names = F,col.names = F, sep="\t", quote=FALSE)
write.table(preds_syn3[,10:13], "truth_syn3.bed",row.names = F,col.names = F, sep="\t", quote=FALSE)

#SYN4
preds_syn4 <- test_set_u |> 
  filter(Sample_Name == 'dream4-T' & PREDICT == TRUE)

truth_syn4 <- test_set_u |> 
  filter(Sample_Name == 'dream4-T' & TRUTH == TRUE)

write.table(preds_syn4[,10:13], "preds_syn4.bed",row.names = F,col.names = F, sep="\t", quote=FALSE)
write.table(preds_syn4[,10:13], "truth_syn4.bed",row.names = F,col.names = F, sep="\t", quote=FALSE)

#SYN5
preds_syn5 <- test_set_u |> 
  filter(Sample_Name == 'dream5-T' & PREDICT == TRUE)

truth_syn5 <- test_set_u |> 
  filter(Sample_Name == 'dream5-T' & TRUTH == TRUE)

write.table(preds_syn5[,10:13], "preds_syn5.bed",row.names = F,col.names = F, sep="\t", quote=FALSE)
write.table(preds_syn5[,10:13], "truth_syn5.bed",row.names = F,col.names = F, sep="\t", quote=FALSE)
