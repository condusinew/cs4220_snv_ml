...
sample_weights <- nrow(df3_median) / sample_counts

#compute mutation type(inverse frequency)
mutation_counts <- table(df3_median$TRUTH)
mutation_weights <- max(mutation_counts) / mutation_counts

#combined case weights w/ sample*mutation
...
#make sure training only has necessary features
```
...
param_grid <- expand.grid(
  cp = c(0.01, 0.05, 0.1, 0.2),        # Complexity paramete 0.01
  maxdepth = c(3, 5, 7, 10),           # Maximum depth of tree 5
  minsplit = c(10, 20, 30, 40),            # Minimum number of observations required for a split 10
