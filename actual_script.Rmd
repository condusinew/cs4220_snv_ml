...
#I NEED TO GET CHROMOS AND SHIT BACK HEREEE
test_set$preds <- final_result_class$predictions
test_set
```
...
# Find the best hyperparameters (highest accuracy)
best_model <- results_df[which.max(results_df$F1), ]
print(best_model)
...
# Make predictions with the best model
final_result_class <- predict(best_model_unweight, test_set)

results_df
# Display confusion xxxxxmatrix for the best model
table(test_set$TRUTH, final_result_class$predictions)
